...
public class ExtensionLoader<T> {
    ...
    private static final String EXTENSION_LOAD_PATH = "META-INF/services/";
    ...
    protected ExtensionLoader(Class<T> interfaceClass, boolean autoLoad,
        ...
        // 接口为空，既不是接口，也不是抽象类
        if (interfaceClass == null || !(interfaceClass.isInterface() || Modifier.isAbstract(
    ...
    private ExtensionClass<T> buildClass(Extension extension, Class<? extends T> implClass,
                                         String alias) {
        ExtensionClass<T> extensionClass = new ExtensionClass<T>(implClass, alias);
        extensionClass.setCode(extension.code());
        extensionClass.setEnable(extension.enable());
    ...
    public T getExtension(String alias) {
        ...
        if (extensionClass == null) {
            throw new CrossWayRuntimeException(LogCodes.getLog(LogCodeNames.ERROR_EXTENSION_NOT_FOUND.toString(),
                    interfaceName, alias));
    ...
    public T getExtension(String alias, Class[] argTypes, Object[] args) {
        ExtensionClass<T> extensionClass = getExtensionClass(alias);
        if (extensionClass == null) {
            throw new CrossWayRuntimeException(LogCodes.getLog(LogCodeNames.ERROR_EXTENSION_NOT_FOUND.toString(), interfaceName, alias));
        } else {
            if (extensible.singleton() && factory != null) {
                T t = factory.get(alias);
                if (t == null) {
            ...
            while (urls.hasMoreElements()) {
                ...
                try {
                    ...
                    if (log.isWarnEnabled()) {
                        log.warn("Failed to load extension of extensible " + interfaceName
    ...
    protected synchronized void loadFromFile(String path) {
        ...
        // 默认如果不指定文件名字，就是接口名
        String file = StringUtils.isBlank(extensible.file()) ? interfaceName :
        ...
        try {
            ...
            if (log.isErrorEnabled()) {
                log.error("Failed to load extension of extensible " + interfaceName + " from path:"
                        + fullFileName, t);
            }
        }
    }
    ...
    protected void readLine(URL url, String line) {
        ...
        try {
            ...
            if (log.isWarnEnabled()) {
                log.warn("Extension {} of extensible {} is disabled, cause by: {}", className,
                        interfaceName, ExceptionUtils.toShortString(e, 2));
            }
            if (log.isDebugEnabled()) {
        ...
        if (!interfaceClass.isAssignableFrom(tmp)) {
            throw new IllegalArgumentException(
                    "Error when load extension of extensible " + interfaceName + " from file:" + url + ", "
                            + className + " is not subtype of interface.");
        }
        ...
        if (extension == null) {
            throw new IllegalArgumentException(
                    "Error when load extension of extensible " + interfaceName + " from file:" + url + ", "
                            + className + " must add annotation @Extension.");
        } else {
            ...
            if (alias == null) {
                ...
                if (!aliasInCode.equals(alias)) {
                    throw new IllegalArgumentException(
                            "Error when load extension of extensible " + interfaceName + " from file:" + url
                                    + ", aliases of " + className + " are " + "not equal between " + aliasInCode
                                    + "(code) and " + alias + "(file).");
                }
            }
            // 接口需要编号，实现类没设置
        ...
        if (old != null) {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug(
                                "Extension of extensible {} with alias {} override from {} to {} failure, "
                                        + "cause by: order of old extension is higher", interfaceName, alias,
                                old.getClazz(), implClass);
                    }
                } else {
                    if (log.isInfoEnabled()) {
                        log.info("Extension of extensible {} with alias {}: {} has been override to {}",
            ...
            // 如果旧扩展是可覆盖的
                ...
                if (old.isOverride() && old.getOrder() >= extension.order()) {
                    // 如果已加载覆盖扩展，再加载到原始扩展
                    if (log.isInfoEnabled()) {
                        log.info("Extension of extensible {} with alias {}: {} has been loaded, ignore "
                                + "origin {}", interfaceName, alias, old.getClazz(), implClass);
                    }
                } else {
                    // 如果不能被覆盖，抛出已存在异常
                    ...
                    if (CommonUtils.isNotEmpty(rejection)) {
                        for (String rej : rejection) {
                            ...
                            if (removed != null) {
                                if (log.isInfoEnabled()) {
                                    log.info(
                                            "Extension of extensible {} with alias {}: {} has been reject "
                                                    + "by new {}",
