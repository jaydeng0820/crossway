...
import crossway.utils.ExceptionUtils;
    ...
    public T getExtension(String alias) {
        ...
        if (extensionClass == null) {
            throw new CrossWayRuntimeException(
                LogCodes.getLog(LogCodeNames.ERROR_EXTENSION_NOT_FOUND.toString(), interfaceName, alias));
        ...
        if (urls != null) {
            while (urls.hasMoreElements()) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Loading extension of extensible {} from classloader: {} and file: {}", interfaceName,
                              classLoader, url);
                }
                ...
                try {
                    ...
                    log.warn(
                        "Failed to load extension of extensible " + interfaceName + " from classloader: " + classLoader
                        + " and file:" + url, t);
                } finally {
                    if (reader != null) {
                        reader.close();
    ...
    private void loadSuccess(String alias, ExtensionClass<T> extensionClass) {
        if (listener != null) {
            try {
                ...
                log.error("Error when load extension of extensible " + interfaceClass + " with alias: " + alias + ".",
                          e);
            }
        } else {
            all.put(alias, extensionClass);
    ...
    protected void readLine(URL url, String line) {
        ...
        try {
            ...
            if (log.isWarnEnabled()) {
                log.warn("Extension {} of extensible {} is disabled, cause by: {}", className, interfaceName,
        ...
        if (!interfaceClass.isAssignableFrom(tmp)) {
            throw new IllegalArgumentException(
                "Error when load extension of extensible " + interfaceName + " from file:" + url + ", " + className
        ...
        if (extension == null) {
            ...
            if (StringUtils.isBlank(aliasInCode)) {
                ...
                throw new IllegalArgumentException(
                    "Error when load extension of extensible " + interfaceClass + " from file:" + url + ", " + className
        ...
        if (old != null) {
            ...
            if (extension.override()) {
                ...
                if (extension.order() < old.getOrder()) {
                    if (log.isDebugEnabled()) {
                        log.debug("Extension of extensible {} with alias {} override from {} to {} failure, "
                                  + "cause by: order of old extension is higher", interfaceName, alias, old.getClazz(),
                                  implClass);
                    }
                } else {
                    if (log.isInfoEnabled()) {
                        log.info("Extension of extensible {} with alias {}: {} has been override to {}", interfaceName,
            ...
            // 如果旧扩展是可覆盖的
                ...
                if (old.isOverride() && old.getOrder() >= extension.order()) {
                    // 如果已加载覆盖扩展，再加载到原始扩展
                    if (log.isInfoEnabled()) {
                        log.info("Extension of extensible {} with alias {}: {} has been loaded, ignore " + "origin {}",
