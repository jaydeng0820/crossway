...
import crossway.annotation.JustForTest;
...
public class CrossWayConfigs {
    ...
    private static void init() {
        try {
            // loadDefault
            String json = FileUtils.file2String(CrossWayConfigs.class, "crass-way-config-default.json", "UTF-8");
            Map map = JsonUtils.toMap(json);
            CFG.putAll(map);

            // loadCustom
            loadCustom("crassway/cross-way-config.json");
            loadCustom("META-INF/crass-way/crass-way-config.json");

            // load system properties
            CFG.putAll(new HashMap(System.getProperties())); // 注意部分属性可能被覆盖为字符串
        } catch (Exception e) {
            throw new CrossWayRuntimeException(LogCodes.getLog(LogCodes.ERROR_LOAD_CROSSWAY_CONFIGS), e);
        }
    }
        ...
        if (urls != null) { // 可能存在多个文件
            ...
            while (urls.hasMoreElements()) {
                ...
                try {
                    ...
                    Map map = JsonUtils.toMap(context.toString());
                    Integer order = (Integer) map.get(CrossWayOptions.RPC_CFG_ORDER);
    ...
    public static void putValue(String key, Object newValue) {
        ...
        if (changed(oldValue, newValue)) {
            ...
            List<CrossWayConfigListener> CrossWayConfigListeners = CFG_LISTENER.get(key);
            if (CommonUtils.isNotEmpty(CrossWayConfigListeners)) {
                for (CrossWayConfigListener CrossWayConfigListener : CrossWayConfigListeners) {
                    CrossWayConfigListener.onChange(oldValue, newValue);
                }
            }
        }
    }
    ...
    synchronized static void removeValue(String key) {
        ...
        if (oldValue != null) {
            ...
            if (CommonUtils.isNotEmpty(CrossWayConfigListeners)) {
                for (CrossWayConfigListener CrossWayConfigListener : CrossWayConfigListeners) {
                    CrossWayConfigListener.onChange(oldValue, null);
                }
            }
        }
    }
    ...
    public static synchronized void subscribe(String key, CrossWayConfigListener listener) {
        List<CrossWayConfigListener> listeners = CFG_LISTENER.get(key);
        if (listeners == null) {
            listeners = new ArrayList<CrossWayConfigListener>();
     ...
     * 用于排序的一个类
     */
    private static class CfgFile implements Ordered {
